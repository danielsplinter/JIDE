...
struct JavaLanguage {
    ...
    // O observador do disco morava aqui, e saiu na fase 4 da `22`: três áreas
    // passaram a querer saber do disco, e o registro no sistema operacional tem
    // de ser um só. Quem avisa esta crate agora é o `file_changed` do contrato.
}
...
";
    ...
    /// Um arquivo que apareceu no disco entra no índice quando alguém avisa.
    ///
    /// **A metade que continua aqui.** O observador saiu desta crate na fase 4
    /// da `22` — três áreas passaram a querer saber do disco, e o registro no
    /// sistema operacional tem de ser um só —, e quem avisa agora é a aplicação.
    /// O que este teste guarda é o outro lado do contrato: avisado, o índice
    /// aprende o arquivo sem varrer o projeto de novo.
    #[test]
    fn o_aviso_de_mudanca_traz_o_arquivo_novo_para_o_indice() {
        ...
        // O arquivo aparece no disco, e a aplicação avisa — que é o que o
        // observador dela faz quando a rajada acaba.
        let externo = root.join("Externo.java");
        assert!(fs::write(&externo, "public class Externo {}\n").is_ok());
        assert!(pollster::block_on(active.file_changed(&externo)).is_ok());
        assert!(
            conhece("Externo"),
            "avisado, o índice aprende o arquivo novo"
        );

        // E apagar tira, pelo mesmo caminho: o aviso não distingue os dois, e o
        // índice descobre no disco qual foi.
        assert!(fs::remove_file(&externo).is_ok());
        assert!(pollster::block_on(active.file_changed(&externo)).is_ok());
    ...
    /// O filtro que decide o que é fonte é **um só**, e é o da varredura.
    ///
    /// Ele era conferido aqui contra o do observador, e os dois eram o mesmo por
    /// construção. O observador saiu desta crate na fase 4 da `22`, e o dele
    /// agora é grosseiro de propósito — a aplicação não sabe o que é fonte de
    /// Java. Este continua sendo o filtro fino, e continua sendo um só: quem
    /// recusa de verdade é o índice, ao reindexar.
    #[test]
    fn o_filtro_da_varredura_decide_o_que_e_fonte() {
        let raiz = PathBuf::from("projeto");
        let fontes = vec![raiz.join("src")];
        for aceito in ["src/Pedido.java", "src/pacote/Servico.java"] {
            assert!(
                crate::analyzer::index::fonte_java(&raiz.join(aceito), &fontes),
        ...
        for recusado in [
            ...
            assert!(
                !crate::analyzer::index::fonte_java(&raiz.join(recusado), &fontes),
                "{recusado} nao deveria interessar"
            );
        }
        // E a poda de pasta é a mesma que a varredura aplica.
        assert!(crate::analyzer::index::caminho_ignorado(&raiz.join("target/classes/A.java")));
        assert!(crate::analyzer::index::caminho_ignorado(&raiz.join(".git/objects/x")));
        assert!(!crate::analyzer::index::caminho_ignorado(&raiz.join("src/A.java")));
    }
