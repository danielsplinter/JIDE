...
use ide_domain::{
    ...
    ProviderId, SemanticSymbol, SymbolKind, TextEdit, TextPosition, TextRange,
};
...
impl TypeScriptServiceProvider {
    ...
    pub(crate) async fn start(
        ...
        let servico = ActiveTypeScriptService {
            ...
            origens: Mutex::new(HashMap::new()),
        };
        // **O que o analisador oferece, ele oferece dentro do escopo** — e por
        // padrão só isso. Um tipo que ainda não foi importado não está no
        // escopo, e a lista vem vazia: medido, `private c: Ht` respondia 38
        // itens com o `import` no topo e **zero** sem ele.
        //
        // `includeCompletionsForModuleExports` é o que faz o analisador olhar
        // para fora do arquivo. É a mesma chave que o VS Code liga, e é o que
        // torna possível oferecer `HttpClient` antes de ele ser importado —
        // desde que a escolha traga o `import` junto, que é o resto desta fase.
        if let Err(erro) = servico
            ...
            .await
        {
            tracing::warn!(%erro, "não foi possível configurar as preferências");
        }
...
pub(crate) struct ActiveTypeScriptService {
    ...
    /// De qual módulo veio cada item da **última** lista respondida.
    ///
    /// # Por que aqui, e não no item
    ///
    /// Para pedir a ação que escreve o `import`, o analisador exige o nome
    /// **e a origem** — `HttpClient` de `@angular/common/http`. O contrato da
    /// IDE carrega só o rótulo, e acrescentar um campo de origem a ele obrigaria
    /// vinte e três construções em cinco crates a falar de uma coisa que só
    /// interessa a quem faz a pergunta seguinte.
    ///
    /// **A última lista basta.** A escolha vem logo depois do pedido — é a
    /// mesma lista que está na tela —, e um rótulo que não esteja aqui é um
    /// item que já estava ao alcance e não precisa de `import` nenhum.
    ///
    /// A **posição** vai junto, e não é detalhe: para calcular onde entra o
    /// `import`, o analisador precisa saber de onde a pergunta veio. Com uma
    /// posição inventada ele responde sem ação nenhuma — verificado, e custou
    /// uma volta.
    origens: Mutex<HashMap<DocumentId, Origens>>,
}

/// De onde veio cada item da última lista, e onde ela foi pedida.
struct Origens {
    line: u32,
    offset: u32,
    /// Como pedir o detalhe de cada item que **não** estava ao alcance.
    ///
    /// Guarda a identificação inteira, e não só o módulo: desde o TypeScript
    /// 4.3 a entrada carrega um `data` opaco, e o analisador exige receber
    /// **ele** de volta para reencontrar o item. Só com o `source`, ele responde
    /// uma lista vazia — verificado, e custou duas voltas.
    ///
    /// Guardar o pedaço de JSON como ele veio é o que impede uma terceira: não
    /// se interpreta o que é opaco, devolve-se.
    de: HashMap<String, serde_json::Value>,
}

/// As trocas de texto que a ação do analisador pede, **só para este arquivo**.
///
/// A resposta traz uma lista de ações, cada uma com mudanças em possivelmente
/// vários arquivos. Escrever num arquivo que não está aberto seria editar às
/// costas de quem trabalha; o que se aplica é o que muda **aqui**.
///
/// As posições vêm em linha e coluna de base um, contadas em unidades do
/// analisador — a mesma conversão que todo o resto deste módulo faz, ao
/// contrário.
fn trocas_da_acao(resposta: &serde_json::Value, path: &Path, texto: &str) -> Vec<TextEdit> {
    let meu_arquivo = mesmo_caminho(path);
    resposta
        .as_array()
        .map(Vec::as_slice)
        .unwrap_or_default()
        .iter()
        .filter_map(|detalhe| detalhe.get("codeActions")?.as_array())
        .flatten()
        .filter_map(|acao| acao.get("changes")?.as_array())
        .flatten()
        .filter(|mudanca| {
            mudanca
                .get("fileName")
                .and_then(serde_json::Value::as_str)
                .is_some_and(|arquivo| mesmo_caminho(Path::new(arquivo)) == meu_arquivo)
        })
        .filter_map(|mudanca| mudanca.get("textChanges")?.as_array())
        .flatten()
        .filter_map(|troca| {
            let novo = troca.get("newText")?.as_str()?.to_owned();
            let inicio = posicao_do_servico(troca.get("start")?, texto)?;
            let fim = posicao_do_servico(troca.get("end")?, texto)?;
            Some(TextEdit {
                range: TextRange {
                    start: inicio,
                    end: fim,
...
/// Uma posição do analisador na numeração do domínio.
fn posicao_do_servico(ponto: &serde_json::Value, texto: &str) -> Option<TextPosition> {
    let linha = u32::try_from(ponto.get("line")?.as_u64()?).ok()?;
    let coluna = u32::try_from(ponto.get("offset")?.as_u64()?).ok()?;
    Some(from_service(Some(texto), linha, coluna))
}

/// O mesmo caminho escrito de duas formas é o mesmo arquivo.
///
/// O analisador devolve o caminho como ele o recebeu, e no Windows isso mistura
/// barra e contrabarra, e maiúscula com minúscula. Comparar texto cru diria que
/// o arquivo aberto e o arquivo da ação são dois.
fn mesmo_caminho(caminho: &Path) -> String {
    caminho
        .to_string_lossy()
        .replace('\\', "/")
        .to_ascii_lowercase()
...
impl ActiveLanguage for ActiveTypeScriptService {
    ...
    async fn completion(
        ...
        let entradas = resposta
            ...
            .cloned()
            .unwrap_or_default();
        let servem: Vec<&serde_json::Value> = entradas
            .iter()
            .filter(|entrada| {
            ...
            .collect();
        // De onde veio cada item que ainda não está ao alcance, para a escolha
        // poder pedir o `import`. Um item sem `source` já está no escopo.
        let origens: HashMap<String, serde_json::Value> = servem
            .iter()
            .filter_map(|entrada| {
                let nome = entrada.get("name")?.as_str()?.to_owned();
                // Sem `source` o item já está no escopo, e não pede `import`.
                let origem = entrada.get("source")?;
                let mut identificacao = serde_json::json!({
                    "name": nome,
                    "source": origem,
                });
                if let Some(dados) = entrada.get("data") {
                    identificacao["data"] = dados.clone();
                }
                Some((nome, identificacao))
            })
            .collect();
        if let Ok(mut registro) = self.origens.lock() {
            registro.insert(
                request.document_id,
                Origens {
                    line,
    ...
    /// O `import` que a escolha deste item exige, se ela exigir um.
    ///
    /// Quem escreve o `import` é o analisador, e não nós: ele sabe se já existe
    /// um do mesmo módulo, onde pôr o novo, e qual estilo de aspas o projeto
    /// usa. Reimplementar isso seria uma segunda resposta para a mesma pergunta,
    /// que envelheceria em silêncio — é a regra que a `23` já aplicou ao
    /// `tsconfig`.
    async fn completion_edits(
        ...
        let Some((line, offset, origem)) = self.origens.lock().ok().and_then(|registro| {
            let guardado = registro.get(&document_id)?;
            let origem = guardado.de.get(&label)?.clone();
            Some((guardado.line, guardado.offset, origem))
        }) else {
            // Sem origem guardada, o item já estava ao alcance: escrever o nome
        ...
        let resposta = self
            ...
            .await
            .map_err(|detalhe| self.failure(detalhe))?;
        Ok(trocas_da_acao(&resposta, &path, &texto))
    }
