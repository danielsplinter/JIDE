...
- [23 — TypeScript](23-typescript.md) — **fases 0 a 5 feitas**, menos dois níveis opcionais de folha de estilo: Java saiu do núcleo, a segunda linguagem existe, o projeto é lido do `tsconfig.json`, o `tsserver` responde com tipo e o `.scss` completa pelo grafo de `@use`; depurar ficou **fora de escopo** — quem depura é o navegador
...
- [25 — Índice próprio de TypeScript](25-typescript-index.md) — **fases 0 a 7 feitas**; a **8 é pendência**: os `lib.*.d.ts` do próprio TypeScript entraram no índice — 1 525 tipos, cache de 410 KB relido em 21 ms contra 1,26 s de análise —, e o `.` sobre `String[]`, `string`, `Date` ou `HTMLElement` responde sem o analisador; **medida, a fase 7 rendeu 2,5 pontos no monorepo e 1 ponto em 397 na aplicação**, e a mesma medição achou onde o ponto morre de verdade: um quarto do que a IDE não responde é o segundo elo de uma cadeia, `this.svc.`, que é a fase 8. O analisador externo só sobe quando o índice diz que não alcança; o `.` declarado responde 14% a 17% dos pontos de um projeto real e diz "não sei" no resto; a busca por tipo responde em 4 ms contra os 30 s do analisador externo, com +4 MB e sem `node_modules`; o analisador externo custa 1,9 GB e 30 s porque guarda o programa inteiro; um índice responde busca, navegação e o `.` **declarado** em dezenas de MB — e diz que não soube no resto, em vez de mentir
