...
- [25 — Índice próprio de TypeScript](25-typescript-index.md) — **fases 0 a 7 feitas**, faltando refazer a medição de cobertura da 7: os `lib.*.d.ts` do próprio TypeScript entraram no índice — 1 525 tipos, cache de 410 KB relido em 21 ms contra 1,26 s de análise —, e o `.` sobre `String[]`, `string`, `Date` ou `HTMLElement` responde sem o analisador. O analisador externo só sobe quando o índice diz que não alcança; o `.` declarado responde 14% a 17% dos pontos de um projeto real e diz "não sei" no resto; a busca por tipo responde em 4 ms contra os 30 s do analisador externo, com +4 MB e sem `node_modules`; o analisador externo custa 1,9 GB e 30 s porque guarda o programa inteiro; um índice responde busca, navegação e o `.` **declarado** em dezenas de MB — e diz que não soube no resto, em vez de mentir
