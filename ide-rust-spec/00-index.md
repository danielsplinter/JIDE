...
- [25 — Índice próprio de TypeScript](25-typescript-index.md) — **fases 0 a 6 feitas**; a **7 é pendência**: trazer os `lib.*.d.ts` do próprio TypeScript para o índice, que é o que falta para o `.` sobre `String[]` não descer ao analisador. O analisador externo só sobe quando o índice diz que não alcança; o `.` declarado responde 14% a 17% dos pontos de um projeto real e diz "não sei" no resto; a busca por tipo responde em 4 ms contra os 30 s do analisador externo, com +4 MB e sem `node_modules`; o analisador externo custa 1,9 GB e 30 s porque guarda o programa inteiro; um índice responde busca, navegação e o `.` **declarado** em dezenas de MB — e diz que não soube no resto, em vez de mentir
